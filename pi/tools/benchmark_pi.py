...
def print_system_info() -> None:
    ...
    if mem:
        print(f"Memory: {mem['used_mb']:.0f} / {mem['total_mb']:.0f} MB ({mem['percent']:.1f}%)")
...
def main():
    ...
    parser.add_argument('--stress-test', type=int, default=0,
                        help='Run stress test for N seconds')
    ...
    for batch_size in batch_sizes:
        ...
        print(f"{batch_size:>8} {stats['mean_us']:>10.2f} {stats['p95_us']:>10.2f} "
    ...
    if args.stress_test > 0:
        ...
        print(f"Batch time: {results['batch_time_mean_ms']:.2f}ms avg, "
              f"{results['batch_time_max_ms']:.2f}ms max")

        if results['temp_start_c']:
            print(f"Temperature: {results['temp_start_c']:.1f}°C → "
