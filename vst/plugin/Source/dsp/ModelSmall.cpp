...
ModelSmall::ModelSmall() {
    ringBuffer_.prepare(SmallConfig::BUFFER_LENGTH);
}

bool ModelSmall::loadModel(const std::string& jsonPath) {
    lastError_.clear();
    ...
    try {
        ...
        if (modelJson.contains("config")) {
            ...
            if (bufferLength != SmallConfig::BUFFER_LENGTH) {
                lastError_ = "Model buffer_length mismatch: expected " +
                             std::to_string(SmallConfig::BUFFER_LENGTH) +
                             ", got " + std::to_string(bufferLength);
                return false;
            }
            if (hiddenSize != SmallConfig::HIDDEN_SIZE) {
                lastError_ = "Model hidden_size mismatch: expected " +
                             std::to_string(SmallConfig::HIDDEN_SIZE) +
                             ", got " + std::to_string(hiddenSize);
                return false;
            }
            if (numLayers != SmallConfig::NUM_LAYERS) {
                lastError_ = "Model num_layers mismatch: expected " +
                             std::to_string(SmallConfig::NUM_LAYERS) +
        ...
        // Small model has 3 dense layers (2 hidden + 1 output)
        if (layers.size() < 3) {
        ...
        // Load layer 0: input -> hidden
        {
            ...
            std::vector<std::vector<float>> weights(SmallConfig::HIDDEN_SIZE,
                std::vector<float>(SmallConfig::INPUT_SIZE));
            std::vector<float> bias(SmallConfig::HIDDEN_SIZE);

            for (int i = 0; i < SmallConfig::HIDDEN_SIZE; ++i) {
                for (int j = 0; j < SmallConfig::INPUT_SIZE; ++j) {
        ...
        // Load layer 1: hidden -> hidden (index 2 in RTNeural after ReLU)
        {
            ...
            std::vector<std::vector<float>> weights(SmallConfig::HIDDEN_SIZE,
                std::vector<float>(SmallConfig::HIDDEN_SIZE));
            std::vector<float> bias(SmallConfig::HIDDEN_SIZE);

            for (int i = 0; i < SmallConfig::HIDDEN_SIZE; ++i) {
                for (int j = 0; j < SmallConfig::HIDDEN_SIZE; ++j) {
                    weights[i][j] = layerWeights[i][j].get<float>();
                }
                bias[i] = layerBias[i].get<float>();
            }
        ...
        // Load layer 2: hidden -> output (index 4 in RTNeural)
        {
            ...
            std::vector<std::vector<float>> weights(SmallConfig::OUTPUT_SIZE,
                std::vector<float>(SmallConfig::HIDDEN_SIZE));
            std::vector<float> bias(SmallConfig::OUTPUT_SIZE);

            for (int i = 0; i < SmallConfig::OUTPUT_SIZE; ++i) {
                for (int j = 0; j < SmallConfig::HIDDEN_SIZE; ++j) {
                    weights[i][j] = layerWeights[i][j].get<float>();
                }
                bias[i] = layerBias[i].get<float>();
...
float ModelSmall::processSample(float input) {
    ringBuffer_.push(input);
    sampleCount_++;

    if (sampleCount_ < SmallConfig::BUFFER_LENGTH || !modelLoaded_) {
        return input;
    }

    float modelOutput = model_.forward(ringBuffer_.data());

    int samplesAfterBufferFill = sampleCount_ - SmallConfig::BUFFER_LENGTH;
    if (samplesAfterBufferFill < SmallConfig::CROSSFADE_LENGTH) {
        float t = static_cast<float>(samplesAfterBufferFill) /
                  static_cast<float>(SmallConfig::CROSSFADE_LENGTH);
...
void ModelSmall::reset() {
    ringBuffer_.reset();
    model_.reset();
    sampleCount_ = 0;
}

void ModelSmall::warmup(int iterations) {
    if (!modelLoaded_) {
        return;
    }

    for (int i = 0; i < SmallConfig::INPUT_SIZE; ++i) {
