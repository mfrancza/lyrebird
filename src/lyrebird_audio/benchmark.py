...
def main():
    ...
    print(
        f"{'Config':<15} {'Params':>10} {'Batch':>6} {'Mean':>10} {'Min':>10} {'Max':>10} {'Realtime':>10}"
    )
    ...
    for buffer_length, hidden_size, num_layers, desc in configs:
        ...
        for batch_size in batch_sizes:
            input_tensor = torch.randn(batch_size, args.channels, buffer_length).to(
                device
            )
            ...
            print(
                f"{desc:<15} {num_params:>10,} {batch_size:>6} {stats['mean_us']:>10.2f} "
    ...
    realtime_configs = [
        r for r in results if r["batch_size"] == 1 and r["realtime_capable"]
    ]
    if realtime_configs:
        for r in realtime_configs:
            headroom = budgets["per_sample_us"] - r["per_sample_us"]
            print(
                f"  {r['config']:<15} {r['params']:>8,} params, {r['mean_us']:>8.2f} μs "
    ...
    print(
        "      Use these numbers as a baseline; expect 2-10x slower on target hardware."
    )
